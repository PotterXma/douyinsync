...
|----------|------------|---------|---------|
| Core | Python | 3.10+（推荐 3.11；CI 验证 3.10–3.13 / Windows） | Main runtime language |
...
| Client | YouTube Data API v3 + OAuth | REST | Upload：`httpx` resumable chunked PUT；鉴权：`google-auth-oauthlib` |
...
> BMAD：规划产物位于 `_bmad-output/planning-artifacts/` 与 `_bmad-output/implementation-artifacts/`；配置见 `_bmad/bmm/config.yaml`。流程与仓库目录映射见 **`docs/bmad-documentation-alignment.md`**。验收以该目录 + **`docs/index.md`** 所列工程文档 + `pytest` 为准。
